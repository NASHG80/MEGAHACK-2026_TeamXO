...
import {
  ...
  Search, X, ArrowUpRight, ExternalLink
} from 'lucide-react';
...
function DetailPanel({ sub, onClose, onToggleSl, showToast, notes, setNotes }) {
  if (!sub) return (
    <div className="h-full flex flex-col items-center justify-center text-center py-16 px-8">
      ...
      <p className="font-semibold text-slate-600 text-sm">Select a registration</p>
      <p className="text-xs text-slate-400 mt-1">Click any row to see details</p>
    </div>
  );
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-5 border-b border-slate-100 flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-extrabold text-slate-800 text-sm leading-snug">{sub.teamName}</h3>
          <p className="text-xs text-slate-400 mt-1">{sub.leaderName} · {sub.college}</p>
        </div>
      ...
      <div className="px-5 py-5 text-center border-b border-slate-100">
        {sub.aiScore != null ? (
          <div className="inline-flex flex-col items-center px-8 py-4 rounded-2xl" style={{ background: sBg(sub.aiScore) }}>
            <span className="text-5xl font-black leading-none" style={{ color: sColor(sub.aiScore) }}>{sub.aiScore}</span>
            <span className="text-xs font-bold mt-1" style={{ color: sColor(sub.aiScore) }}>{sLabel(sub.aiScore)}</span>
            <span className="text-xs text-slate-400 mt-0.5">AI Score / 100</span>
          </div>
        ) : (
          <p className="text-sm text-slate-400 italic">No resume uploaded — score unavailable</p>
        )}
      </div>
      ...
      {sub.resumeUrl && (
        <div className="px-5 py-3 border-b border-slate-100">
          <a href={`http://localhost:5000${sub.resumeUrl}`} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 text-xs font-semibold text-royal hover:underline">
      ...
      <div className="px-5 py-5 flex-1 overflow-y-auto border-b border-slate-100">
        ...
        <textarea rows={3} value={notes[sub._id] || ''} onChange={e => setNotes(n => ({ ...n, [sub._id]: e.target.value }))}
          ...
          placeholder="Private note about this registration…" />
      </div>

      {/* Actions */}
      <div className="p-5 flex flex-col gap-2.5">
        <button onClick={() => onToggleSl(sub._id)}
          ...
          {sub.shortlisted ? 'Remove from Shortlist' : 'Shortlist This Team'}
        </button>
      </div>
    </div>
  );
...
/* ─── SUBMISSIONS TABLE ─── */
function SubmissionsPane({ subs, setSubs, showToast, selected, setSelected, onToggleSl }) {
  const [threshold, setThreshold] = useState(85);
  const [search, setSearch]       = useState('');

  const filtered = [...subs]
    .filter(s => !search || s.teamName?.toLowerCase().includes(search.toLowerCase()) || s.leaderName?.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => (b.aiScore ?? -1) - (a.aiScore ?? -1));

  const autoShortlist = async () => {
    const eligible = subs.filter(s => (s.aiScore ?? 0) >= threshold && !s.shortlisted);
    if (!eligible.length) { showToast('No new teams above threshold'); return; }
    const token = localStorage.getItem('hf_token');
    await Promise.all(eligible.map(s =>
      fetch(`http://localhost:5000/api/registrations/shortlist/${s._id}`, {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search team or leader…"
      ...
      <div className="space-y-2 flex-1 overflow-y-auto pr-1">
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <p className="text-sm text-slate-400 italic">No registrations found.</p>
          </div>
        )}
        {filtered.map((s, i) => {
          ...
          return (
            <div key={s._id} onClick={() => setSelected(isSelected ? null : s)}
              ...
              <div className="min-w-0">
                <p className={`font-bold text-sm truncate ${isSelected ? 'text-white' : 'text-slate-800'}`}>{s.teamName}</p>
                <p className={`text-xs truncate mt-0.5 ${isSelected ? 'text-blue-200' : 'text-slate-400'}`}>{s.leaderName} · {s.college}</p>
              </div>
                ...
                {score != null ? (
                  <>
                    <span className={`text-lg font-extrabold leading-none ${isSelected ? 'text-white' : ''}`}
                      style={isSelected ? {} : { color: sColor(score) }}>
                      {score}
                    </span>
                    <span className={`text-[10px] mt-0.5 font-semibold ${isSelected ? 'text-blue-200' : 'text-slate-400'}`}>/100</span>
                  </>
                ) : (
                  <span className={`text-[10px] italic ${isSelected ? 'text-blue-200' : 'text-slate-300'}`}>N/A</span>
                )}
              </div>
              ...
              <div className="hidden md:flex justify-center">
                {s.shortlisted
                  ...
                  : <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${isSelected ? 'bg-white/20 text-blue-100' : 'bg-slate-100 text-slate-400'}`}>{score != null ? sLabel(score) : 'No Resume'}</span>
                }
              </div>

              {/* Shortlist toggle */}
              <div className="flex justify-end" onClick={e => { e.stopPropagation(); onToggleSl(s._id); }}>
      ...
      <div className="space-y-2.5 mb-6">
        {sl.map((s, i) => (
          <div key={s._id}
            ...
            <div className="w-10 h-10 rounded-xl flex items-center justify-center font-extrabold text-sm shrink-0" style={{ background: sBg(s.aiScore ?? 0), color: sColor(s.aiScore ?? 0) }}>
              #{i+1}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-slate-800 text-sm truncate">{s.teamName}</p>
              <p className="text-xs text-slate-400 mt-0.5">{s.leaderName} · {s.college}</p>
            </div>
            <div className="flex items-baseline gap-1 shrink-0">
              <span className="text-xl font-extrabold" style={{ color: sColor(s.aiScore ?? 0) }}>{s.aiScore ?? '—'}</span>
...
export default function PptReview() {
  ...
  const scoredSubs = subs.filter(s => s.aiScore != null);
  const avgScore = scoredSubs.length ? Math.round(scoredSubs.reduce((a, s) => a + s.aiScore, 0) / scoredSubs.length) : '—';
  const topScore = scoredSubs.length ? Math.max(...scoredSubs.map(s => s.aiScore)) : '—';

  const toggleSl = async (id) => {
    const s = subs.find(x => x._id === id);
    const token = localStorage.getItem('hf_token');
    if (!s.shortlisted) {
      await fetch(`http://localhost:5000/api/registrations/shortlist/${id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
    }
    setSubs(p => p.map(x => x._id === id ? { ...x, shortlisted: !x.shortlisted } : x));
    if (selected?._id === id) setSelected(prev => prev ? ({ ...prev, shortlisted: !prev.shortlisted }) : null);
    showToast(s.shortlisted ? `${s.teamName} removed from shortlist` : `${s.teamName} shortlisted ✓`);
  };
