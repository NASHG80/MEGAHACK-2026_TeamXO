...
exports.getDashboard = async (req, res) => {
  try {
    ...
    // Fetch pending SOS requests for this hackathon from EventSOS
    const EventSOS = require('../models/EventSOS');
    const sosQuery = h
      ? { hackathonId: h._id }
      : (member.hackathon_id && mongoose.Types.ObjectId.isValid(member.hackathon_id)
          ? { hackathonId: member.hackathon_id } : {});
    const helpRequests = await EventSOS.find({ ...sosQuery, $or: [{ cocomResolved: false }, { studentResolved: false }] })
      .sort({ createdAt: -1 });
